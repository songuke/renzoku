...
unsigned int morton_box(BoundingBox *box, BoundingBox *scene_aabb) {
    Vec3 c = (box->v_min + box->v_max) * 0.5f;
    c = (c - scene_aabb->v_min) / (scene_aabb->v_max - scene_aabb->v_min);    // shift to [0, 1]
    return Nvidia::morton3D(c.x(), c.y(), c.z());
}

unsigned int morton_point(Vec3 p, BoundingBox *scene_aabb) {
    p = (p - scene_aabb->v_min) / (scene_aabb->v_max - scene_aabb->v_min); 
    return Nvidia::morton3D(p.x(), p.y(), p.z());
}

int morton_compare(const void * a, const void * b)
{
    return ( (int)(((MortonEntry*)a)->code) - (int)(((MortonEntry*)b)->code) );
}

void sort_morton_array(unsigned int *morton, int n, MortonEntry *&entries) {
